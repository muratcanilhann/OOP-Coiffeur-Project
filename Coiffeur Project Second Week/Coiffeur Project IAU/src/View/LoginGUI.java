...
public class LoginGUI extends JFrame {
	...
	private JTextField fld_cuspass;
	...
	public LoginGUI() {
		...
		fld_cuspass = new JTextField();
		fld_cuspass.setColumns(10);
		fld_cuspass.setBounds(150, 68, 220, 24);
		w_customerlogin.add(fld_cuspass);
				...
				if(fld_stfid.getText().length() == 0 || fld_stfpass.getText().length() == 0) {
					...
					try {
						...
						while(rs.next()) {
							...
							if(fld_stfid.getText().equals(rs.getString("identitynum")) && fld_stfpass.getText().equals(rs.getString("password"))){
